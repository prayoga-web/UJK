...
<div class="bg-gradient-to-r from-indigo-600 to-violet-600 rounded-2xl p-8 mb-8 text-white shadow-lg shadow-indigo-200 relative overflow-hidden">
    <div class="relative z-10">
        <h2 class="text-3xl font-bold mb-2">Tambah Kategori Baru</h2>
        <p class="opacity-80">Tambahkan kategori untuk menata barang berdasarkan tipe produk.</p>
    </div>
...
<div class="max-w-2xl mx-auto bg-white p-8 rounded-3xl shadow-xl shadow-slate-200/60 border border-slate-100">
    <form action="{{ route('categories.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="nama_kategori" value="{{ old('nama_kategori') }}" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-700 focus:ring-2 focus:ring-indigo-500 focus:bg-white outline-none transition-all" placeholder="Contoh: Smartphone" required>
        </div>
